...
const { default: CommandPaletteBody } = await import("./commandPaletteBody.tsx");

test("renders nothing while closed", () => {
  const html = renderToStaticMarkup(
    React.createElement(CommandPaletteProvider, null, React.createElement(CommandPaletteBody))
  );
  assert.equal(html, "");
});
