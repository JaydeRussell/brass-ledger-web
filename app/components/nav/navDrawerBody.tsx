...
/**
 * The left-hand hamburger menu's actual contents. Loaded lazily by
 * navDrawer.tsx (which is what app/layout.tsx mounts) — this module
 * pulls in ui/dialog.tsx's Radix Dialog, ~37 KB that used to sit on
 * every route's critical path including /login, for a panel nobody has
 * opened yet.
 *
 * Built on ui/dialog.tsx's Radix-backed
...
export default function NavDrawerBody() {
  ...
  // Reads the one shared CurrentUserProvider (see lib/auth.ts) — no
  // fetch of its own. Only used here to role-gate the Admin link.
