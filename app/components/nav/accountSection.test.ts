...
// ../../lib/auth), which reads the shared CurrentUserProvider — mocked
// out here the same way this project's other hook-heavy components are
// (see navDrawer.test.ts's note on why one shared mutable variable, set
// per test, is used instead of calling mock.module() again for each case).
...
// AccountSection's sign-out handler closes the drawer via useNav(), which
// throws outside a NavProvider — so, like every other context in this
// codebase, the real provider is used rather than mocked (see
// navDrawer.test.ts / commandPalette.test.ts).
function render() {
  return renderToStaticMarkup(
    React.createElement(NavProvider, null, React.createElement(AccountSection))
  );
}
