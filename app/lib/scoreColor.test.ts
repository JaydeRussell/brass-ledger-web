...
// Table-driven: every branch of classifyScore (draw / win / loss),
// including the zero-zero edge case.
test("classifyScore", () => {
  const cases: { name: string; reference: number; other: number; want: ScoreOutcome }[] = [
    ...
    { name: "zero-zero is a draw", reference: 0, other: 0, want: "draw" },
    { name: "a big win is a win", reference: 90, other: 10, want: "win" },
    { name: "a big loss is a loss", reference: 10, other: 90, want: "loss" },
    { name: "a narrow win is still a win", reference: 55, other: 45, want: "win" },
    { name: "a narrow loss is still a loss", reference: 45, other: 55, want: "loss" },
  ];
...
test("SCORE_OUTCOME_CLASSES has a class for every ScoreOutcome classifyScore can produce", () => {
  const outcomes: ScoreOutcome[] = ["loss", "draw", "win"];
